...
| Containerization | Backend only, from V1; SQLite in a named volume; no DB service until Phase 4; frontend not containerized. Gunicorn is the image's prod default (`config.wsgi`); `docker-compose.override.yml` swaps in runserver for dev (auto-reload via a source bind mount). migrate runs in both modes via the entrypoint | Dev-prod parity for the backend down to the app server (same image artifact as the VPS); SQLite suffices in V1, so a Postgres container would be ballast (added additively in Phase 4); the frontend ships as a static build, so it runs locally via npm |
| Frontend–backend transport | CORS (django-cors-headers), not a Vite dev proxy | Production frontend (all-inkl) and backend (VPS) are cross-origin anyway, so CORS is needed regardless; a dev proxy would only work in dev |
| Repo layout | Monorepo, separate `backend/` + `frontend/` | Separate toolchains (Python/pip vs. Node/npm) and separate deployment targets (VPS vs. all-inkl); solo project, so one repo beats two |
...
| Data source | RESOLVED: MD Manager extract (graph/recipes) + in-game rates | Test/dry-run uses placeholder values (source irrelevant). **Shipping source resolved:** the recipe graph, building assignment, byproducts, tool-wear, item prices and resident consumption values come from `notes/md_manager_extract.json` (extracted from MD Manager v1.3 — Xarmo / r00t @ Toplitz Discord, Nexus mod 25; license permits reuse **with credit** — keep attribution). NOT in that source, curated separately: production **rates** (`output_per_day_at_100`) — measured in-game at skill 3; building **levels / max_workers** — e.g. from the wiki (sewing hut 1/1/2). The extract is a *curation source*, not the shippable dataset: it uses display names (map to snake_case IDs), targets MD Manager v1.3's game version (verify against current game before shipping), and field processes (`production_day_flag: 1`) are not V1 producers. **Datamining the `.pak` was investigated and rejected** (see `spike/datamine_spike.md`): current pak (v1.4.x) is AES-encrypted, key not in FModel's official repo, obtaining it leads to piracy-adjacent aggregators or DRM-stripping — disproportionate. The in-game screen carries an unavoidable worker-mood bonus (~70–80, can't be zeroed), so it is an approximate rate cross-check. Values are facts (reusable with credit); raw game assets are never redistributed. |
| MD Manager data: keep local | `backend/planner/game_data.json` is gitignored, not in the public repo (for now) | Attribution stays in the file header; on publication show a visible credit in the tool footer; consider asking the author (Xarmo / r00t @ Toplitz Discord) for explicit permission. Consequence: the repo only runs where the file is present locally; a checked-in example/fallback would be needed before a fresh clone works. |
| Static data curation (first pass) | Core subset from the MD Manager extract into `backend/planner/game_data.json` | A hand-curated connected subset across the 8 buildings (real recipes/inputs/byproducts from the source), loaded by `data.py` into `STATIC`. English snake_case IDs (display names slugified, e.g. "Flax Stalk"→`flax_stalk`); field renamed `rate_at_100`→`output_per_day_at_100`, kept **null** (rates measured in-game) with the calc null-safe and a `rates_missing` flag surfaced in the UI. `max_workers` only set for `sewing_hut` (1/1/2), null elsewhere (TODO, not guessed); per-recipe level gating also TODO. Field processes (`production_day_flag==1`) excluded; their outputs stay producer-less leaves. `byproducts` stored but **not yet evaluated** by calc/graph (data only). Reusable-tool inputs dropped (tool wear out of V1 scope). `verified: false` + attribution kept; values target MD Manager v1.3, unchecked against the current game. `resident_consumption` left as placeholder (`food`/`water`) — not derivable from the source. |
